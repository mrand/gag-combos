...
import { resetCog, setCog, toggleV2, toggleLured } from './cog.slice';
...
function LuredToggle() {
  ...
  return (
    <div className='toggle lured-toggle'>
      {
        (isLured) ? (
...
function V2Toggle() {
  const isV2 = useSelector((state) => state.cog.isV2);
  const dispatch = useDispatch();

  return (
    <div className='toggle v2-toggle'>
      {
       isV2 ? (
